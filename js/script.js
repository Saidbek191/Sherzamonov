document.addEventListener("DOMContentLoaded", () => {
  ...
  posts.forEach((post) =>
    post.addEventListener("click", () => updateLeft(post)),
  );
  updateLeft(posts[0]);
  ...
  const btn = document.querySelector(
    `.lang__switch[data-lang="${currentLang}"]`,
  );
  ...
  function loadArticle(lang) {
    ...
    if (articleContainer) {
      ...
      fetch(`articles/${currentArticlesSlug}/${lang}.md`)
        ...
        .catch(
          () =>
            (articleContainer.innerHTML = `<p>Перевод для языка "${lang}" недоступен.</p>`),
        );
    }
  }
});
